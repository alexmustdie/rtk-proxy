...
class Message:
    ...
    def parseParam(data):
        ...
        fields['name'] = data[5:37].decode('utf-8').replace('\0', '')
        return fields
...
class Protocol:
    ...
    def unpack(typeValue, data, count=1):
        try:
            ...
            if typeValue == 0: # bool
                if len(data) >= count:
                    return tuple([True if data[i] != 0 else False for i in range(0, count)])
                else:
                    return None
            else:
                return None
...
class File:
    ...
    def finalizeWrite(self, total):
        timeout = self.component.messenger.defaultTimeout
    ...
    def writeImpl(self, data, chunkSize, burstSize, append, verify):
        ...
        while state.position < state.total and state.result.value == Result.SUCCESS:
            ...
            while position < state.total and position - state.position < int(chunkSize * burstSize / 2):
                ...
                self.writeFileChunk(position, data[position:position + nextChunkSize])
        ...
        if verify:
            ...
            if size != state.total or checksum != binascii.crc32(data) & 0xFFFFFFFF:
                raise ChecksumError()
...
class Hub:
    ...
    def getParamList(self):
        ...
        for i in range(0, self.getParamCount()):
            ...
            if param is not None:
                self.parameters[i] = (param['name'], param['value'])
                verbose('Parameter {:d} "{:s}", value {:g}'.format(i, param['name'], param['value']))
            else:
                break
    ...
    def onParamReceived(self, value, fields, callback=None, number=None, name=None):
        ...
        if fields['id'] == Message.PARAM_INFO:
            ...
            if (number is not None and number == fields['number']) or (name is not None and name == fields['name']):
                if value is None or numpy.isclose(value, fields['value']):
    ...
    def getProtocolInfo(self, version=(1, 5), callback=None, blocking=True):
    ...
    def sendCommand(self, code, callback=None, blocking=True):
        ...
        if not blocking or callback is not None:
            self.messenger.invokeAsync(packet=request, callback=proxyCallback, retries=1)
        else:
            response = self.messenger.invoke(packet=request, callback=proxyCallback, retries=1)
            return Result(response['result'])

    def onCommandResponseReceived(self, fields, callback=None):
        if callback is not None:
            callback(Result(fields['result']))
        return True
        ...
        def handle(self):
            if self.callback is None:
                ...
                for i in range(0, self.retries):
                    ...
                    proxyCallback = lambda fields: Messenger.messageCallback(state, self.callback, fields)
                    startTime = time.time()
                    self.messenger.send(self.packet, proxyCallback)
                    timer = threading.Timer(self.timeout, Messenger.GenericRequest.timerCallback, [state])
                    timer.start()
                    state.sem.acquire()
                    endTime = time.time()
                    timer.cancel()
                    self.messenger.forget(proxyCallback)

                    if state.response is not None:
                        self.messenger.latency = ([endTime - startTime] + self.messenger.latency)[:100]
                        break

        @staticmethod
        def timerCallback(state):
            state.sem.release()
    ...
    def invoke(self, packet, callback=None, retries=None, timeout=None):
        if callback is None:
            ...
            for i in range(0, retries):
                ...
                startTime = time.time()
                self.send(packet, proxyCallback)
                timer = threading.Timer(timeout, Messenger.timerCallback, [state])
                timer.start()
                state.sem.acquire()
                endTime = time.time()
                timer.cancel()
                self.forget(proxyCallback)

                if state.response is not None:
                    self.latency = ([endTime - startTime] + self.latency)[:100]
